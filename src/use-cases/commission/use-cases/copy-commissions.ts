...
export async function copyCommissions(
  ...
  try {
    ...
    const sourceCommissions = await repository.findCommissionsInRange(
      srcStart,
      srcEnd,
    );
    if (sourceCommissions.length === 0) throw new NoEntriesToCopyError();

    const createdCount = await repository.replaceCommissionsInRange(
      targetStart,
      targetEnd,
      sourceCommissions,
    );
    ...
    return {
      success: false,
      error: "Falha ao copiar comissões do mês selecionado.",
    };
  }
}
