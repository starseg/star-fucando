...
import {
  assertDifferentReferenceMonth,
  toReferenceMonthRange,
} from "@/domain/shared/value-objects/reference-month";
...
export async function copyMealVouchers(
  ...
  try {
    ...
    const { start: srcStart, end: srcEnd } = toReferenceMonthRange(
      sourceMonth,
      sourceYear,
    );
    const { start: targetStart, end: targetEnd } = toReferenceMonthRange(
      targetMonth,
      targetYear,
    );

    const sourceVouchers = await repository.findMealVouchersInRange(
      srcStart,
      srcEnd,
    );
    if (sourceVouchers.length === 0) throw new NoEntriesToCopyError();

    const createdCount = await repository.replaceMealVouchersInRange(
      targetStart,
      targetEnd,
      sourceVouchers,
    );
    ...
    return {
      success: false,
      error: "Falha ao copiar vales alimentação do mês selecionado.",
    };
  }
}
